...
public final class MaxHeap<T extends Comparable<T>> {
    ...
    public MaxHeap(Collection<T> initialValues) {
    ...
    public List<T> toSortedDescendingList() {
        MaxHeap<T> copy = new MaxHeap<T>(values);
    ...
    private void siftDown(int index) {
        ...
        while (true) {
            ...
            if (left < values.size() && compare(values.get(left), values.get(largest)) > 0) {
                largest = left;
            }
            if (right < values.size() && compare(values.get(right), values.get(largest)) > 0) {
                largest = right;
            }
            if (largest == current) {
                return;
